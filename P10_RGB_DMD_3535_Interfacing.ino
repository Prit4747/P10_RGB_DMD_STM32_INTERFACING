...
inline VirtualCoords CustomPxBasePanel ::getCoords(int16_t x, int16_t y) {
  ...
  uint8_t pxbase = 8;  // pixel base - common values are 4, 8, or 16 for quarter-scan panels
...
void loop() {
  ...
  // Display static HELLO text from (0,0) - top left
  FourScanPanel->setTextSize(1);        // Normal 1:1 pixel scale (5x7 font)
  FourScanPanel->setTextColor(FourScanPanel->color565(255, 0, 0)); // White color
  FourScanPanel->setCursor(0, 0);       // Position at (0,0) - top left
  FourScanPanel->print("HELLO");
  
  // Display RGB colors statically at row 8 (lower area)
  // R in Red color
  FourScanPanel->setTextColor(FourScanPanel->color565(255, 0, 0)); // Red color
  FourScanPanel->setCursor(0, 8);  // Start from row 8
  FourScanPanel->print("R");
  
  // G in Green color
  FourScanPanel->setTextColor(FourScanPanel->color565(0, 255, 0)); // Green color
  FourScanPanel->setCursor(6, 8);  // Position at row 8, x=6
  FourScanPanel->print("G");
  
  // B in Blue color
  FourScanPanel->setTextColor(FourScanPanel->color565(0, 0, 255)); // Blue color
  FourScanPanel->setCursor(12, 8);  // Position at row 8, x=12
  FourScanPanel->print("B");
  
  delay(25); // Small delay for smooth animation
} // end loop